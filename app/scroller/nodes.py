...
class ScrollerNodes:
    ...
    def _generate_issue_details_with_llm(self, titles: list, state: dict):
        ...
        지정된 로컬 LLM(3B)을 최우선으로 사용하되, 할루시네이션(환각) 방지 프롬프트를 강력하게 적용합니다.
        """
        try:
            prompt = f"""
            ...
            [작성 규칙]
            1. 반드시 아래와 같은 JSON 형식으로만 응답할 것 (마크다운 없이 순수 JSON 텍스트).
            {{
            ...
            2. 🚨 [치명적 중요도] 할루시네이션 절대 금지: 제공된 기사 제목에 없는 인물(예: 이명박, 박근혜 등)이나 사실을 절대로 지어내거나 추론해서 적지 마십시오. 오직 제공된 제목 텍스트 안에 있는 인물 이름(예: 이재명, 윤석열, 한동훈 등)과 팩트만 사용하십시오!
