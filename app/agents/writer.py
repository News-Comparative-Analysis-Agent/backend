...
class WriterAgent:
    ...
    def node_write_draft(self, state: ComparisonState) -> dict:
        ...
        - evidence의 판단·해석·문제 제기 문장들을 이 문단에 모두 담는다.

        [두 번째 문단 형식 - 같은 언론사 후속]
        - 반드시 아래 형식을 따른다:
        "[언론사]는 '[결론 문장]'이라며 '[요구·주장 문장]'라고 했다."
        - 날짜와 사설 제목을 반복하지 않는다.
        - evidence의 결론·요구·주장 문장들을 이 문단에 모두 담는다.
        - evidence 마지막 문장까지 반드시 포함한다. 생략은 규칙 위반이다.
        ...
        [인용 연결 규칙]
        - 같은 문단 내 여러 문장 연결: '~라며' 사용
        - 같은 언론사 다음 문단 시작: "[언론사]는 '...'이라며"
        - 대립 논조 전환: "반면"으로 시작
        - "또한", "아울러", "이어" 등 다른 연결어는 절대 사용하지 않는다.
