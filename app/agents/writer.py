...
class WriterAgent:
    ...
    def node_write_draft(self, state: ComparisonState) -> dict:
        ...
        판단·해석(evidence_front): {mv.get('evidence_front', '')}
        결론·요구(evidence_back): {mv.get('evidence_back', '')}
        발행일: {date_str}
        기사제목: {article_title}
        """ 
        ...
        3. **원문 문장을 단 한 글자도 바꾸지 마라.**
           - [언론사별 분석 데이터]의 `판단·해석(evidence_front)` 및 `결론·요구(evidence_back)` 문장만 인용한다.
           - 요약하거나 표현을 바꾸는 것은 금지한다.
           - 데이터에 없는 문장을 새로 만들거나 추가하는 것은 금지한다.
        ...
        6. **각 언론사는 1~2개 문단으로 작성한다.**
           - 첫 번째 문단: 해당 언론사의 판단, 해석, 문제 제기를 담은 `evidence_front` 문장들로 구성한다.
           - 두 번째 문단: 해당 언론사의 결론, 요구, 주장을 담은 `evidence_back` 문장들로 구성한다.
           - 사건 설명 문장(언제, 누가, 무엇을 했는지)은 언론사 문단에 포함하지 않는다.
           - 원문에 없는 문장을 생성하는 것은 금지한다.
        ...
        [첫 번째 문단 형식 - 해당 언론사 첫 등장]
        - 반드시 아래 형식을 따른다:
        "[언론사]는 [날짜] 사설 <[기사제목]>에서 '[판단·해석 문장]'이라며 '[문제 제기 문장]'라고 했다."
        - 큰따옴표 없이 작은따옴표('')로 인용한다.
        - 하나의 문단 안에서 '~라며' 로 앞 문장과 뒷 문장을 연결한다.
        - `evidence_front`의 판단·해석·문제 제기 문장들을 이 문단에 모두 담는다.

        [두 번째 문단 형식 - 같은 언론사 후속]
        - 반드시 아래 형식을 따른다:
        "[언론사]는 '[결론 문장]'이라며 '[요구·주장 문장]'라고 했다."
        - 날짜와 사설 제목을 반복하지 않는다.
        - `evidence_back`의 결론·요구·주장 문장들을 이 문단에 모두 담는다.
        - `evidence_back` 마지막 문장까지 반드시 포함한다. 생략은 규칙 위반이다.

        [대립 논조 전환]
        - 반드시 "반면 [언론사]는 사설 <[기사제목]>에서..." 로 시작한다.
        - 날짜는 생략한다.

        [인용 연결 규칙]
        - 같은 언론사 다음 문단 시작: "[언론사]는 '...'이라며"
        - 대립 논조 전환: "반면"으로 시작

        [사건 설명 문장 금지]
        - 판결 확정, 기소 여부, 발언 경위 등 팩트 나열 문장은 언론사 문단에 포함하지 않는다.
        - (2)번 사건 경위에서 이미 다뤘으므로 재사용 금지.
        ...
        - 원문에 없는 사실 추가 또는 문장 생성 금지
        - 언론사 원문 문장 요약 또는 변형 금지
        - `evidence_front`나 `evidence_back` 내용을 임의로 생략하거나 축약하는 것 금지
        - 마무리·결론 문단 작성 금지
