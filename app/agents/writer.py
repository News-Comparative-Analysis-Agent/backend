...
class WriterAgent:
    ...
    def node_write_draft(self, state: ComparisonState) -> dict:
        ...
        6. **분량을 충분히 써라 — 축약 금지.**
           - 각 언론사의 `원문 근거(evidence)`에 담긴 내용을 최대한 빠짐없이 인용한다.
           - evidence 텍스트가 길 경우, 2~3개의 단락으로 나눠 이어서 전부 담아라.
           - 한 언론사당 최소 2개 단락(첫 인용 단락 + 후속 단락) 이상 작성한다.
           - 분량을 줄이기 위해 evidence 내용을 생략하거나 합치는 것은 금지한다.
        ...
        (3) 언론사별 입장:
            ...
            - 해당 언론사의 `원문 근거(evidence)` 전체 내용을 2~3개 단락으로 나눠 빠짐없이 인용한다.
            - 첫 번째 단락: "[언론사]는 [날짜] 사설 <제목>에서 '...'라고 했다."
            - 이후 단락: "[언론사]는 '...'라고 했다." (사설·날짜 재반복 없이)
