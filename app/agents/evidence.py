...
class EvidenceAgent:
    ...
    def _extract_single_card(self, art: dict, issue_id: int, llm_mode: str, state: ComparisonState) -> tuple[dict | None, dict]:
        ...
        5. "narrative": 해당 언론사가 이 사건을 어떤 관점에서 정의하고 있는지(예: '정치적 공작' vs '진실 규명')를 분석적으로 기술하는 1~2문장의 서술을 작성하세요.

        [출력 JSON 예시]
        {{
            "thought": "조선일보는 이 사건을 '진술 회유'라는 본질적 의혹 대신 '녹취록 공개의 절차적 정당성' 문제로 치환하고 있다. 이는 민주당의 폭로를 정치적 공작으로 몰아가려는 전형적인 프레임 전환 전략이다.",
            "claim": "민주당의 녹취록 일부 공개는 선거용 짜깁기일 가능성이 높으므로 전문 공개 없이는 신뢰할 수 없다.",
            "evidence": "정치적 이유가 있는 것 아니냐는 의구가 생길 수밖에 없다. 이런 의문을 없앨 방법은 간단하다. 녹취록 전문을 공개하면 된다.",
