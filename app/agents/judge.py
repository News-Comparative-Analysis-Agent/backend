...
class JudgeAgent:
    ...
    def node_evaluate_draft(self, state: ComparisonState) -> dict:
        ...
        try:
            ...
            [작업 지시사항]
            1. 반드시 JSON 형식으로만 응답해야 합니다. 마크다운(` ```json `)을 쓰지 마세요.
            2. 먼저 위 세 가지 기준에 대한 각각의 평가 사유(Thought)를 3~4문장으로 상세히 적으세요. 특히 팩트체크 원문에 없는 거짓이 있다면 구체적으로 명시하여 감점하세요.
            3. 기준에 따라 총점(total_score, 0~100)을 합산하여 매기세요.
            4. 만약 총점이 70점 미만이라면, Editor가 개선해야 할 명확한 지시사항(redo_instruction)을 남기세요. 70점 이상이면 빈 문자열.
