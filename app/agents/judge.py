    ...
    def node_evaluate_draft(self, state: ComparisonState) -> dict:
        ...
        try:
            ...
            for m in media_views:
                ...
                claim = m.get("claim", "")
                ev_front = m.get("evidence_front", "")
                ev_back = m.get("evidence_back", "")
                raw_content = m.get("raw_content", "")[:1500] # 토큰 절약을 위해 1500자 제한
                
                ev_text = f"핵심주장: {claim}\n판단·해석: {ev_front}\n결론·요구: {ev_back}\n기사 원본(맥락 참고용): {raw_content}...".strip()
                if claim or ev_front or ev_back:
            ...
            [평가 기준 (Rubric)]
            1. 팩트 정합성 (30점): 기사 본문의 인용 문장이 위 [언론사별 원문 근거]의 '판단·해석' 및 '결론·요구' 문장과 한 글자도 틀리지 않고 일치하는가? 원문에 없는 사실을 추가하거나 요약·변형하지 않았는가? 각 언론사의 인용문이 타 언론사 원문과 뒤섞이지 않았는가? (원문 변형 및 섞임 1건당 -10점 감점)
            2. 형식 준수 (30점): 각 언론사 소개가 "{{언론사}}는 {{날짜}} 사설 <{{제목}}>에서 '...'라고 했다." 형식을 따르는가? 같은 언론사의 두 번째 단락은 "{{언론사}}는 '...'라고 했다." 형식인가? 대립하는 시각은 "반면"으로 전환했는가? 각 언론사마다 [판단·해석] 문단과 [결론·요구] 문단이 엄격히 분리되어 인용되었는가? (형식 위반 및 단락 축약 1건당 -5점 감점)
            3. 구조 완결성 (20점): 리드(행위 주체 명시 + 갈등 요약 원문 그대로) → 사건 경위(반박 포함) → 언론사별 입장 순서를 지켰는가? 마무리 문단 없이 마지막 언론사 인용으로 끝맺었는가? 기자 해석·논평 문장("이처럼", "결국", "이는 ~을 의미한다" 등)이 없는가? (논평 문장 포함 시 -10점 감점)
            4. 원본 충실도 (20점): 기사에 인용된 부분들이 원래 언론사가 보도하고자 했던 전체 맥락이나 뉘앙스(기사 원본 참고)를 왜곡하거나 심각하게 편향되게 다루지 않았는가? 문맥적 의도를 충실히 반영했는가? (맥락 왜곡 1건당 -10점 감점)
            ...
            - 각 언론사당 evidence 분량이 충분하지 않으면(2단락 미만이거나 evidence 내용이 일부만 인용된 경우) 형식 준수 점수를 감점하고, redo_instruction에 누락된 evidence 내용을 명시하십시오.
            - [중요] 응답은 반드시 마크다운(```json)이나 다른 설명 텍스트 없이, 순수한 JSON 객체 하나만 출력하십시오.

            [응답 예시]
            성공예시 (PASS):
            {{
              "thought": "1. 팩트 정합성: 조선일보 인용문 '이런 의문을 없앨 방법은 간단하다. 녹취록 전문을 공개하면 된다'가 원문과 일치함. 경향신문 인용도 원문과 동일함. 2. 형식 준수: '[언론사]는 사설에서...' 형식을 모든 언론사에 일관되게 사용했고, '반면'으로 진보·보수 시각을 전환함. 각 언론사마다 2개 이상 단락으로 evidence를 충분히 인용함. 3. 구조 완결성: 리드에 갈등 구도를 담고, 사건 경위 → 언론사별 입장 순서를 정확히 따름. 기자 의견·해석 문장 없음.",
              "total_score": 91,
              "redo_instruction": ""
            }}
            ...
            반려예시 (FAIL):
            {{
              "thought": "1. 팩트 정합성: 한겨레 인용 부분에서 원문의 '검찰 수사는 재판에서 무죄가 나든 말든'이 '검찰이 무죄 판결에도 불구하고'로 변형되어 있음. 2. 형식 준수: 경향신문이 1개 단락만 인용되어 evidence의 나머지 내용('검사가 야당 대표를 옭아매기 위해...')이 누락됨. 3번 언론사(경향신문) 소개 시 '[경향신문]에 따르면...' 형식을 사용해 '[경향신문]는 사설에서...' 형식을 위반함. 3. 구조 완결성: 마지막 단락에 '이는 언론의 자기 역할 망각을 보여주는 것이다'라는 기자 해석 문장이 포함되어 있음.",
              "total_score": 52,
              "redo_instruction": "첫째, 한겨레 인용 부분을 원문 근거의 '검찰 수사는 재판에서 무죄가 나든 말든, 일단 야당 대표를 기소해서 사법적 족쇄를 채우려는 의도가 있었던 것으로 의심된다'로 교체하십시오. 둘째, 경향신문 단락을 추가하여 원문 근거의 '검사가 야당 대표를 옭아매기 위해 형량거래를 시도하고 거짓 진술을 압박했다면 두말할 것도 없이 검찰권을 남용한 중대 범죄이다' 문장을 '경향신문은 ...'라고 했다.' 형식으로 인용하십시오. 셋째, 경향신문 소개 문장을 '경향신문은 사설에서...' 형식으로 수정하십시오. 넷째, 마지막 단락의 '이는 언론의 자기 역할 망각을 보여주는 것이다' 문장을 삭제하십시오."
            }}
            """
