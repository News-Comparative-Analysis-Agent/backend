...
class JudgeAgent:
    ...
    def node_evaluate_draft(self, state: ComparisonState) -> dict:
        ...
        try:
            ...
            2. 형식 준수 (30점): 각 언론사 소개가 "{{언론사}}는 {{날짜}} 사설 <{{제목}}>에서 '...'라고 했다." 형식을 따르는가? 같은 언론사의 두 번째 단락은 "{{언론사}}는 '...'라고 했다." 형식인가? 대립하는 시각은 "반면"으로 전환했는가? 각 언론사의 evidence 내용이 충분히 인용되었는가(언론사당 최소 2개 단락 이상, 생략·축약 없이)?
