...
class IssueAgent:
    ...
    def node_structure_issues(self, state: ComparisonState) -> dict:
        ...
        if article_mode == "politics":
            prompt = f"""
            당신은 정치적 현안 뉴스 데이터를 바탕으로 사건의 실질적인 핵심 쟁점을 분석하는 미디어 전문가입니다.
            아래 [언론사별 분석 데이터]를 바탕으로, 언론사들이 이 사건을 보도하면서 독자들에게 공통적으로 전달하고자 하는 가장 본질적인 핵심 쟁점을 객관적으로 요약하십시오.
            ...
            2. 기자 자신의 의견이나 주관적 판단을 쓰지 마십시오. 각 분석 데이터의 `핵심주장(claim)`과 `원문 근거(evidence)`를 객관적으로 분석하여 서술하십시오.
            3. 언론사 기사들이 집중하여 전달하려 하는 가장 핵심적인 쟁점/의혹 1가지를 명료하게 도출하십시오.
 
            [conflict_summary 작성 지침]
            - 이 문장은 기사의 리드(두 번째 문장)에 수정 없이 그대로 삽입된다.
            - 반드시 아래 형식의 한 문장으로만 작성한다. 두 문장 이상 쓰면 규칙 위반이다.
              "이번 사안을 둘러싼 핵심 쟁점은 '[언론사들이 전달하고자 하는 핵심 쟁점]'이다."
            - 따옴표 안은 명사형 또는 명사구로 간결하고 담백하게 작성한다. (쟁점이 꼭 2개 이상일 필요는 없으며, 보도를 통해 사회적으로 부각된 가장 지배적인 현안/의혹을 명확히 짚어내야 한다.)
 
            [출력 JSON 예시]
            {{
                "conflict_summary": "이번 사안을 둘러싼 핵심 쟁점은 '야당 대표를 겨냥한 집단 테러 모의의 실체 및 신변보호 시급성'이다."
            }}
        """
            ...
            [conflict_summary 작성 지침]
            - 이 문장은 기사 서두(리드의 두 번째 문장)에 수정 없이 그대로 삽입됩니다.
            - 주요 언론사들의 전반적인 반응을 한두 문장으로 간결하고 자연스럽게 서술하십시오.
            - 실제 언론사들의 반응(공통점, 차이점, 강조점 등)을 있는 그대로 요약합니다.
            - 예시: "주요 일간지들은 ~에 한목소리를 내면서도, ~에 대해서는 시각차를 보였다.", "언론들은 일제히 ~을 촉구했다.", "주요 일간지의 반응은 ~와 ~로 엇갈렸다." 등 상황에 맞게 유연하게 서술합니다.
